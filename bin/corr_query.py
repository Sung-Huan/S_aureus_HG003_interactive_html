        ...
        if info != q_info:
            ...
            if "positive" in filename:
                if (float(spearmanr(exps[index], q_exp[index_q])[0]) >= cutoff):
                    ...
                    ids.append(info)
                    ccs.append("{0:.5f}".format(float(spearmanr(exps[index],
                                                      q_exp[index_q])[0])))
...
def main():
    ...
    for srna in srnas:
        ...
        print("_".join([srna.feature, str(srna.start),
                        str(srna.end), srna.strand]))
        output_file("_".join([srna.feature, str(srna.start), str(srna.end),
                             srna.strand]) + ".html", mode='inline')
